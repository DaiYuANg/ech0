...
func (s *Store) Join(nodeID, addr string) error {
	s.logger.Info("received join request for remote node %s at %s", nodeID, addr)
	...
	for _, srv := range configFuture.Configuration().Servers {
		...
		if srv.ID == raft.ServerID(nodeID) || srv.Address == raft.ServerAddress(addr) {
			...
			if srv.Address == raft.ServerAddress(addr) && srv.ID == raft.ServerID(nodeID) {
				s.logger.Info("node %s at %s already member of cluster, ignoring join request", nodeID, addr)
				return nil
			}
	...
	s.logger.Info("node %s at %s joined successfully", nodeID, addr)
	return nil
}
