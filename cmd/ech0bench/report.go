...
func printReport(
	...
	writeStdout("mode=%s data_dir=%s broker_addr=%s topic=%s duration=%s producers=%d consumers=%d partitions=%d payload_bytes=%d fetch_batch=%d\n",
		cfg.mode(), dataDir, cfg.brokerAddr, cfg.topic, elapsed.Round(time.Millisecond), cfg.producers, cfg.activeConsumers, cfg.partitions, cfg.payloadBytes, cfg.fetchBatch)
