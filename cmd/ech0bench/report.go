...
func printReport(
	...
	writeStdout("mode=%s data_dir=%s broker_addr=%s topic=%s duration=%s producers=%d consumers=%d partitions=%d payload_bytes=%d batch_size=%d producer_inflight=%d fetch_batch=%d commit_every=%d\n",
		cfg.mode(), dataDir, cfg.brokerAddr, cfg.topic, elapsed.Round(time.Millisecond), cfg.producers, cfg.activeConsumers, cfg.partitions, cfg.payloadBytes, cfg.batchSize, cfg.producerInflight, cfg.fetchBatch, cfg.commitEvery)
